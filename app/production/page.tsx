...
export default function ProductionPage() {
  const service = servicePages.production;
    ...
    <main className="min-h-screen bg-cream text-charcoal">
      ...
      <section className="py-20 sm:py-28">
        <div className="mx-auto grid max-w-7xl gap-8 px-5 sm:px-8 lg:grid-cols-[1.2fr_0.8fr] lg:px-10">
          ...
          <aside className="rounded-[2rem] bg-sage p-7">
            <Icon className="h-10 w-10 text-brick" />
            <p className="mt-10 font-mono text-xs uppercase tracking-[0.22em] text-brick">
              {service.signal}
            </p>
            <h2 className="mt-6 text-3xl font-black">Inputs</h2>
            <p className="mt-3 leading-7 text-charcoal/70">{service.input}</p>
            <h2 className="mt-8 text-3xl font-black">Outputs</h2>
            <p className="mt-3 leading-7 text-charcoal/70">{service.output}</p>
          </aside>
        </div>
      </section>
      <ContactBand />
